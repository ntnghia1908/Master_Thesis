...
from utility.helper import list2csv
import wandb
...
class WandbCallback_bestModel(BaseCallback):
    ...
    def __init__(
        ...
        super(WandbCallback_bestModel, self).__init__(verbose)
    ...
    def _on_training_end(self) -> None:
        if self.model_save_path is not None:
            self.save_model(last_model=True)
        # wandb.log({"best_reward": self.best_mean_reward})
        wandb.log({"best_reward": self.best_mean_reward})
        x, y = ts2xy(load_results(self.log_dir), 'timesteps')
        list2csv(self.model_save_path+'/reward', y)

    def save_model(self, last_model=False) -> None:
        if last_model is True:
            self.model.save(self.path + 'last')
            self.model.save(os.path.join(wandb.run.dir, "model_last.h5"))
        else:
            self.model.save(self.path)
            self.model.save(os.path.join(wandb.run.dir, "model_best.h5"))
            if self.verbose > 1:
