import math
import numpy as np
from utils import VideoListCollector
...
class SinglepathEnvGym(gym.Env):
    ...
    # Youtube recommend
    UTILITY_SCORE = np.array([700, 900, 2000, 3000, 5000, 6000, 8000])  # for 04-second segment
    VIDEO_BIT_RATE = np.array([700, 900, 2000, 3000, 5000, 6000, 8000])  # for 04-second segmen

    SEGMENT_SPACE = 7  # Window size
    ...
    def __init__(self, log_qoe=True, bitrate_list=None, replace=True, train=True):
        if log_qoe:
            ...
            self.REBUF_PENALTY = 2.6  # 1 sec rebuffering -> 3 Mbps
        else:
        ...
        obs_space = dict(
            network_speed=gym.spaces.Box(low=0, high=float("inf"), shape=(self.HISTORY_SIZE,), dtype=np.float32),
            next_chunk_size=gym.spaces.Box(low=0, high=float("inf"),
                                           shape=(self.QUALITY_SPACE,), dtype=np.float32),
            ...
            delay=gym.spaces.Box(low=0, high=float("inf"), shape=(self.HISTORY_SIZE,), dtype=np.float32),
        )
    ...
    def reset(self):
        ...
        if self.train:
            ...
            self.eps_counter += 1
            self.eps_counter = self.eps_counter % 200
            self.init_net_seg = 0
        ...
        self.rebuffer_time_simu = 0.0  # rebuffering time for episode, calculated from simulation
        self.rebuffer_time = 0.0  # rebuffering time for episode, calculated from formula

        self.est_throughput = 0.0

        self.total_reward = 0.0  # reward of episode
        self.reward_quality_norm = 0.0  # reward_quality of episode, normalized by M_IN_K
        self.reward_smooth_norm = 0.0  # smooth reward of episode, normalied by M_IN_K
        self.reward_rebuffering_norm = 0.0  # rebuffering time reward of episode, normalized by M_IN_K
        ...
        self.event = np.concatenate((self.event,
                                     [[delay, self.EVENT.DOWN_FINISH, 0, self.DEFAULT_QUALITY]]
                                     ))
        ...
        # Add play event
        self.event = np.concatenate((self.event,
                                     [[delay + 0.00001, self.EVENT.PLAY, 0, self.DEFAULT_QUALITY]]
                                     ))
        ...
        self.delay_net = np.zeros(self.HISTORY_SIZE)  # Download time of previous 6 chunks, for this part
        self.state = dict(
            network_speed=self.network_speed,
            next_chunk_size=np.zeros(self.QUALITY_SPACE, ),
    ...
    def down_time(self, segment_size, cur_time):
        ...
        while True:  # download segment process finish after a full video segment is downloaded
            ...
            if max_throughput > segment_size:  # finish download in network segment
                ...
                delay += self.NETWORK_SEGMENT - seg_time_stamp  # delay from begin in second
    ...
    def step(self, action):
        ...
        self.event = np.concatenate(
            (self.event, [[cur_down_time + delay, self.EVENT.DOWN_FINISH, down_id, down_quality]]))
        ...
        while True:
            ...
            if self.event[0][1] == self.EVENT.DOWN:
                if self.pick_next_segment() is None:
                    self.event = np.concatenate((self.event,
                                                 [[cur_time + self.SAMPLE, self.EVENT.SLEEP_FINISH, -1, -1]]))
            ...
            if self.event[0][1] == self.EVENT.DOWN_FINISH:
                ...
                self.buffer_size_trace = max(0, self.buffer_size_trace - delay) + self.VIDEO_CHUNK_LEN

                if self.buffer_size_trace > self.BUFFER_THRESHOLD or (sum(self.download_segment < -0.5) == 0):
                    self.event = np.concatenate((self.event,
                                                 [[cur_time + self.SAMPLE, self.EVENT.SLEEP_FINISH, -1, -1]]))
                else:
                    self.event = np.concatenate((self.event, [[cur_time + 0.00001, self.EVENT.DOWN, -1, -1]]))

            if self.event[0][1] == self.EVENT.SLEEP_FINISH:  # this make an infinite loop
                chunk_sleep_time += self.SAMPLE
                self.buffer_size_trace = max(0, self.buffer_size_trace - self.SAMPLE)
            ...
            if self.event[0][1] == self.EVENT.PLAY:
                ...
                self.event = np.concatenate(
                    (self.event,
                     [[cur_time + self.VIDEO_CHUNK_LEN, self.EVENT.PLAY_FINISH, self.play_id, play_quality]]))
            ...
            if self.event[0][1] == self.EVENT.PLAY_FINISH:
                ...
                if self.download_segment_finish[self.play_id + 1] < -0.5:
                    self.event = np.concatenate((self.event, [
                        [cur_time + self.SAMPLE, self.EVENT.FREEZE_FINISH, self.play_id + 1,
                         -1]]))  # waiting for play_id+1 chunk
                else:
                    self.event = np.concatenate((self.event, [
                        [cur_time + 0.00001, self.EVENT.PLAY, self.play_id + 1,
                         self.download_segment_finish[self.play_id + 1]]]))

            if self.event[0][1] == self.EVENT.FREEZE_FINISH:
                self.rebuffer_time_simu += self.SAMPLE
                if self.download_segment_finish[int(self.event[0][2])] < -0.5:  # next chunk has not downloaded yet
                    self.event = np.concatenate((self.event, [
                        [cur_time + self.SAMPLE, self.EVENT.FREEZE_FINISH, self.event[0][2],
                         -1]]))  # waiting for event[0][2]
                else:
                    self.event = np.concatenate((self.event, [
                        [cur_time + 0.00001, self.EVENT.PLAY, self.event[0][2],
        ...
        chunk_reward_smooth = np.abs(self.UTILITY_SCORE[down_quality] - \
                                     self.UTILITY_SCORE[last_down_quality])
        if down_quality != last_down_quality:
            self.num_switch += 1

        chunk_reward_norm = chunk_reward_quality * self.REWARD_PENALTY / 100 \
        ...
        self.total_reward = self.reward_quality_norm - self.reward_smooth_norm - self.reward_rebuffering_norm

        # Calculate new state
        next_chunk_size = self.video_list[:, down_id+1 % self.SEGMENT_SPACE] if down_id + 1 < self.CHUNK_TIL_VIDEO_END \
        ...
        self.state = dict(
            ...
            percentage_remain_video_chunks=np.array([remain / self.CHUNK_TIL_VIDEO_END]),
            last_down_quality=int(self.download_segment[down_id]),
            delay=self.delay_net,
        )
        if self.play_id == self.CHUNK_TIL_VIDEO_END - 1:
            # print("Check rebuffer_time: simu = ", self.rebuffer_time_simu, \
            #       ", formula = ", self.rebuffer_time, \
            #       ", different = ", self.rebuffer_time_simu - self.rebuffer_time)
