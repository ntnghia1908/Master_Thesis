...
class PensieveFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 256, use_feature_extractor=True):
        ...
        if use_feature_extractor:
            ...
            self.last_layer = nn.Sequential(nn.Linear(2872, self.features_dim * 2), nn.Tanh(),
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        cat = torch.cat((network_speed1, network_speed2, next_chunk_size, quality_in_buffer, buffer_size,
                         percentage_remain_video_chunks, last_play_quality, delay_net1, delay_net2), dim=1)
